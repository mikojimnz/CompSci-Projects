...
public class SimplePicture implements DigitalPicture {
 ...
 /////////////////////// Constructors /////////////////////////
    ...
    * must be given in order for a class to be able to be subclassed. By default
    * all subclasses will implicitly call this in their parent's no-argument
   ...
   public SimplePicture(File theFile) {
      try {
         this.loadOrFail(theFile);
      } catch (Exception ex) {
         fileName = "";
         System.out.println("There was an error trying to load " + fileName+ ":\n" + ex.toString());
    ...
    *
    * @param copyMe the picture to copy from
    */
   public SimplePicture(SimplePicture copyMe) {
      if (copyMe.fileName != null) {
         this.fileName = new String(copyMe.fileName);
         this.extension = copyMe.extension;
         this.title = copyMe.getTitle();
      }
      if (copyMe.bufferedImage != null) {
         this.bufferedImage = new BufferedImage(copyMe.getWidth(),
                 copyMe.getHeight(), BufferedImage.TYPE_INT_RGB);
         this.copyPicture(copyMe);
      }
   }
 ...
 ////////////////////////// Methods //////////////////////////////////
   ...
   /**
    * Method to get a Graphics2D object for this picture which can be used to do
    * 2D drawing on the picture
    */
   ...
   public void loadOrFail(String fileName) throws IOException {
      ...
      //System.out.println("File constructed: " + file.getAbsolutePath());
      if (!file.canRead()) {
   ...
   /**
    * Method to load the picture from the passed file reference.
    * Added to make it easier to use a FileChooserDialog with pictures.
    * @param aFile the file name to use to load the picture from
    * @author L. LaSpina
    * @throws java.io.IOException 
    */
   public void loadOrFail(File aFile) throws IOException {
      // set the current picture's file name
      ...
      if (posDot >= 0) {
         ...
         title = fileName.substring(0,posDot);
      }
      else {
         title = fileName;
         extension = "jpeg"; //default guess.
      }
   ...
   public void writeOrFail(String fileName) throws IOException {
      ...
      //System.out.println("Dir = " + fileLoc);
   ...
   /**
    * Method to write the contents of the picture to a given File.
    * 
    * 
    * @param file The file path to store this image.
    * @return true if all went well, false if the file type (given by the extension)
    * is not supported.
    */
